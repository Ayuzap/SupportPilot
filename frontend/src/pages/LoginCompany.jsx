      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-2">
          ...
          <p className="text-sm text-slate-500">
            Access your SupportPilot workspace to manage products, documents, and support analytics.
          </p>
        </CardHeader>
        <CardContent>
          {!isSupabaseConfigured && (
            <div className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
              Demo auth is active. Company and customer roles are stored in browser local storage.
            </div>
          )}
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <Button className="w-full" disabled={isSubmitting} size="lg" type="submit">
              {isSubmitting ? "Signing in..." : "Log in as company"}
            </Button>
          </form>

          <p className="mt-5 text-center text-sm text-slate-500">
            Need a company workspace?{" "}
            <Link className="font-medium text-indigo-600 hover:text-indigo-700" to="/register/company">
