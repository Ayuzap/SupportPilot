...
function RegisterCompany() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await register({ ...form, role: "company" });
    ...
    <div className="page-container flex min-h-[70vh] items-center justify-center">
      <Card className="w-full max-w-lg">
        <CardHeader className="space-y-2">
          <CardTitle className="text-2xl">Create a company workspace</CardTitle>
          <p className="text-sm text-slate-500">
            Set up a support portal for your products, documentation, and service teams.
          </p>
        </CardHeader>
        <CardContent>
          {!isSupabaseConfigured && (
            <div className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
              Demo auth is active. This will create a company account in local browser storage.
            </div>
          )}
